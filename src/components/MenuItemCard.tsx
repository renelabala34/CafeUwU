...
export default function MenuItemCard({ item, onViewDetail }: MenuItemCardProps) {
  ...
  return (
    <div className="group bg-white rounded-2xl shadow-sm hover:shadow-xl border border-tomato-100/60 overflow-hidden transition-all duration-300 hover:-translate-y-1 flex flex-col">
      {/* Emoji / Visual */}
      <div className="relative aspect-square bg-gradient-to-br from-cream-100 to-warm-100 flex items-center justify-center">
        <span className="text-5xl sm:text-6xl md:text-7xl group-hover:scale-110 transition-transform duration-500">
          {item.emoji}
        </span>
      ...
      <div className="p-3 sm:p-4 md:p-5 flex flex-col flex-1">
        <h3 className="font-bold text-tomato-900 text-xs sm:text-sm md:text-base leading-tight mb-0.5 sm:mb-1 line-clamp-2">
          {item.name}
        </h3>
        <p className="text-[10px] sm:text-xs text-tomato-500 mb-2 sm:mb-3 truncate">{item.category}</p>
        <p className="text-[10px] sm:text-xs text-tomato-600/70 line-clamp-2 mb-3 sm:mb-4 flex-1 hidden sm:block">
          {item.description}
        </p>
        ...
        <div className="flex items-center justify-between mt-auto pt-2 sm:pt-3 border-t border-tomato-100/60">
          <div>
            <span className="text-base sm:text-lg md:text-xl font-black text-tomato-700">
              ${item.price}
            </span>
            <span className="text-[10px] sm:text-xs text-tomato-400 ml-0.5 sm:ml-1">CUP</span>
          </div>
          <button
